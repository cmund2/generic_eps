...
*/
int32_t GENERIC_EPS_CommandDevice(i2c_bus_info_t *device, uint8_t cmd,
                                  uint8_t value) {
  ...
  if (cmd < 0xAB) {
    ...
    i2c_master_transaction(device, GENERIC_EPS_CFG_I2C_ADDRESS, write_data, 3,
                           NULL, 0, GENERIC_EPS_CFG_I2C_TIMEOUT);
  } else {
    status = OS_ERROR;
#ifdef GENERIC_EPS_CFG_DEBUG
    OS_printf("  GENERIC_EPS_CommandDevice: Command 0x%02x is above valid "
              "range! (>= 0xAB expected) \n",
              cmd);
#endif
  }
...
*/
int32_t GENERIC_EPS_RequestHK(i2c_bus_info_t *device,
  ...
  if (calc_crc == read_data[GENERIC_EPS_DEVICE_HK_LEN]) {
    ...
    for (uint8_t i = 0; i < 8; i++) {
      data->Switch[i].Voltage =
          (read_data[offset] << 8) | read_data[offset + 1];
      data->Switch[i].Current =
          (read_data[offset + 2] << 8) | read_data[offset + 3];
      data->Switch[i].Status =
          (read_data[offset + 4] << 8) | read_data[offset + 5];
      offset = offset + 6;
    }
  } else {
    status = OS_ERROR;
#ifdef GENERIC_EPS_CFG_DEBUG
    OS_printf("  GENERIC_EPS_RequestHK: CRC error, expected 0x%02x and "
              "received 0x%02x \n",
...
** Command EPS Switch
*/
int32_t GENERIC_EPS_CommandSwitch(i2c_bus_info_t *device, uint8_t switch_num,
                                  uint8_t value,
                                  GENERIC_EPS_Device_HK_tlm_t *data) {
  int32_t status = OS_SUCCESS;
  ...
  if (switch_num < 8) {
    ...
    if ((value == 0x00) || (value == 0xAA)) {
...
#ifdef GENERIC_EPS_CFG_DEBUG
            OS_printf(
                "  GENERIC_EPS_CommandSwitch: HK reported incorrect switch "
                "state after command! (0x%02x expected, 0x%02x actual) \n",
      ...
      status = OS_ERROR;
#ifdef GENERIC_EPS_CFG_DEBUG
      OS_printf("  GENERIC_EPS_CommandSwitch: Value number of 0x%02x is "
                "invalid! (0x00 or 0xAA expected) \n",
                value);
#endif
    }
  } else {
    status = OS_ERROR;
#ifdef GENERIC_EPS_CFG_DEBUG
    OS_printf("  GENERIC_EPS_CommandSwitch: Switch number of %d is invalid! "
              "(>= 8 expected) \n",
              switch_num);
#endif
  }
