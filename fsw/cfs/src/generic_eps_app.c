...
void EPS_AppMain(void) {
  ...
  */
  while (CFE_ES_RunLoop(&GENERIC_EPS_AppData.RunStatus) == true) {
    ...
    */
    status =
        CFE_SB_ReceiveBuffer((CFE_SB_Buffer_t **)&GENERIC_EPS_AppData.MsgPtr,
                             GENERIC_EPS_AppData.CmdPipe, CFE_SB_PEND_FOREVER);
    ...
    */
    if (status == CFE_SUCCESS) {
      ...
      CFE_EVS_SendEvent(GENERIC_EPS_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "GENERIC_EPS: SB Pipe Read Error = %d", (int)status);
      GENERIC_EPS_AppData.RunStatus = CFE_ES_RunStatus_APP_ERROR;
    }
  }
...
int32 GENERIC_EPS_AppInit(void) {
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_EPS: Error registering for event services: 0x%08X\n",
        (unsigned int)status);
    return status;
  }
  ...
  */
  status = CFE_SB_CreatePipe(&GENERIC_EPS_AppData.CmdPipe,
                             GENERIC_EPS_PIPE_DEPTH, "EPS_CMD_PIPE");
  if (status != CFE_SUCCESS) {
    CFE_EVS_SendEvent(GENERIC_EPS_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(GENERIC_EPS_CMD_MID),
                            GENERIC_EPS_AppData.CmdPipe);
  if (status != CFE_SUCCESS) {
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(GENERIC_EPS_REQ_HK_MID),
                            GENERIC_EPS_AppData.CmdPipe);
  if (status != CFE_SUCCESS) {
  ...
  ** telemetry that has been defined in the GENERIC_EPS_HkTelemetryPkt for this
  *app.
  */
  ...
  if (status != OS_SUCCESS) {
    ...
    */
    status = CFE_EVS_SendEvent(
        GENERIC_EPS_STARTUP_INF_EID, CFE_EVS_EventType_INFORMATION,
        "GENERIC_EPS App Initialized. Version %d.%d.%d.%d",
        GENERIC_EPS_MAJOR_VERSION, GENERIC_EPS_MINOR_VERSION,
        GENERIC_EPS_REVISION, GENERIC_EPS_MISSION_REV);
    if (status != CFE_SUCCESS) {
      CFE_ES_WriteToSysLog(
          "GENERIC_EPS: Error sending initialization event: 0x%08X\n",
          (unsigned int)status);
    }
  }
  return status;
...
void GENERIC_EPS_ProcessCommandPacket(void) {
  ...
  /*
  ** Ground Commands with command codes fall under the GENERIC_EPS_CMD_MID
  *(Message ID)
  */
  ...
  */
  default:
    ...
    CFE_EVS_SendEvent(GENERIC_EPS_PROCESS_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
                      "GENERIC_EPS: Invalid command packet, MID = 0x%x",
...
void GENERIC_EPS_ProcessGroundCommand(void) {
  ...
  */
  case GENERIC_EPS_NOOP_CC:
    ...
    */
    if (GENERIC_EPS_VerifyCmdLength(GENERIC_EPS_AppData.MsgPtr,
                                    sizeof(GENERIC_EPS_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      /* Second, send EVS event on successful receipt ground commands*/
      CFE_EVS_SendEvent(GENERIC_EPS_CMD_NOOP_INF_EID,
  ...
  */
  case GENERIC_EPS_RESET_COUNTERS_CC:
    if (GENERIC_EPS_VerifyCmdLength(GENERIC_EPS_AppData.MsgPtr,
                                    sizeof(GENERIC_EPS_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_EPS_CMD_RESET_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
  ...
  */
  case GENERIC_EPS_SWITCH_CC:
    if (GENERIC_EPS_VerifyCmdLength(GENERIC_EPS_AppData.MsgPtr,
        ...
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_EPS_CMD_SWITCH_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
                        "GENERIC_EPS: Switch command received");
      status = GENERIC_EPS_CommandSwitch(
          &GENERIC_EPS_AppData.Generic_epsI2C,
          ((GENERIC_EPS_Switch_cmd_t *)GENERIC_EPS_AppData.MsgPtr)
              ->SwitchNumber,
      ...
      if (status == OS_SUCCESS) {
        ...
        CFE_EVS_SendEvent(
            GENERIC_EPS_SWITCH_INF_EID, CFE_EVS_EventType_INFORMATION,
        ...
        CFE_EVS_SendEvent(
            GENERIC_EPS_SWITCH_ERR_EID, CFE_EVS_EventType_ERROR,
  ...
  */
  default:
    ...
    CFE_EVS_SendEvent(GENERIC_EPS_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
                      "GENERIC_EPS: Invalid command code for packet, MID = "
                      "0x%x, cmdCode = 0x%x",
...
void GENERIC_EPS_ReportHousekeeping(void) {
  ...
  status = GENERIC_EPS_RequestHK(&GENERIC_EPS_AppData.Generic_epsI2C,
                                 &GENERIC_EPS_AppData.HkTelemetryPkt.DeviceHK);
  if (status == OS_SUCCESS) {
    GENERIC_EPS_AppData.HkTelemetryPkt.DeviceCount++;
  } else {
  ...
  CFE_SB_TransmitMsg((CFE_MSG_Message_t *)&GENERIC_EPS_AppData.HkTelemetryPkt,
                     true);
  return;
}
...
*/
int32 GENERIC_EPS_VerifyCmdLength(CFE_MSG_Message_t *msg,
  ...
  if (expected_length == actual_length) {
    ...
    CFE_EVS_SendEvent(
        GENERIC_EPS_LEN_ERR_EID, CFE_EVS_EventType_ERROR,
